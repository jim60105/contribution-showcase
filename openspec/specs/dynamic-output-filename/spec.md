...
Derives the output HTML filename from the TOML `title` field when no explicit
output path is configured, applying filesystem-safe sanitization with fallback
to `index.html`.
