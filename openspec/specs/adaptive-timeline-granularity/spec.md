...
The system SHALL compute the date span between the earliest and latest commit dates and select the timeline granularity using a five-level cascade. At each level, count the number of distinct buckets that would be produced; if ≤ `timeline_max_buckets` (default: 14), use that granularity:
