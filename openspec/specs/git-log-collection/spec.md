...
- **GIVEN** a project named `"VMS.wiki"` and a commit subject `"feat(nav): add sidebar"`
...
- **GIVEN** a project named `"VMS.wiki"` and a commit subject `"Update architecture page"`
- **WHEN** the system parses the commit
- **THEN** `commit_type` is `"docs"` and `scope` is `""`

#### Scenario: Commit in a .wiki project that is already docs

- **GIVEN** a project named `"VMS.wiki"` and a commit subject `"docs: update readme"`
- **WHEN** the system parses the commit
- **THEN** `commit_type` is `"docs"` (unchanged) and `scope` is `""`

#### Scenario: Commit in a non-wiki project is not overridden

- **GIVEN** a project named `"VMS-Frontend"` and a commit subject `"feat: add map"`
- **WHEN** the system parses the commit
- **THEN** `commit_type` is `"feat"` (not overridden)

#### Scenario: Project name that contains .wiki but does not end with it

- **GIVEN** a project named `"VMS.wiki-tools"` and a commit subject `"feat: add parser"`
- **WHEN** the system parses the commit
- **THEN** `commit_type` is `"feat"` (not overridden)

#### Scenario: Case-sensitive suffix match

- **GIVEN** a project named `"VMS.WIKI"` and a commit subject `"feat: add page"`
- **WHEN** the system parses the commit
