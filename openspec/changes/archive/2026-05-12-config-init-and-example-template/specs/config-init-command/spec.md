# Capability: config-init-command

This capability adds an `init` subcommand to the contribution-showcase CLI that
bootstraps a well-documented template configuration file (`showcase.toml`) in the
current working directory. The template content is embedded at compile time from
